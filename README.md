...
## vLLM/SGLang: model, tensor parallelism, and storage

Launch (and the Templates admin form) has three pieces built specifically
for LLM-serving templates, though none of them are actually vLLM/SGLang-
specific — they're generic primitives any template's `args` can use:

- **Model** — a dedicated field, separate from the free-text `args` box.
  Substituted for `{{model}}` in `args`, so vLLM's seeded args are just
  `--model={{model}}`. Nothing about the field cares whether the value is
  a Hugging Face model ID or a local filesystem path — it's a plain string
  substitution either way; what makes a path actually resolve inside the
  container is the storage mount below.
