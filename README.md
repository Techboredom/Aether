...
- **Model**, **context length**, and **quantization** — dedicated fields,
  separate from the free-text `args` box, substituted for `{{model}}`,
  `{{context_length}}`, and `{{quantization}}` respectively (so vLLM's
  seeded args include `--model={{model}}`, `--max-model-len={{context_length}}`,
  `--quantization={{quantization}}`). `model` doesn't care whether it's a
  Hugging Face ID or a local filesystem path — it's a plain string
  substitution either way; what makes a path actually resolve inside the
  container is the storage mount below. All three are genuinely optional:
...
- **Tensor parallelism matches whatever was requested** — there's no
  ...
  nonsensical 0 — unlike the three above, this one is never dropped).
  vLLM's full seeded args:
  `--model={{model}} --tensor-parallel-size={{accelerator_count}}
  --max-model-len={{context_length}} --quantization={{quantization}}`;
  SGLang's: `--model-path={{model}} --host=0.0.0.0 --port=30000
