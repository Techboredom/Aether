...
- **Launch** — creates a `Deployment` in that namespace, owned by whichever
  ...
  intended (and, for RStudio, only) way in (JupyterLab, RStudio). The name
  you type is only your own — the actual Deployment/Service is named
  `<your username>-<name>`, so it only needs to be unique among what
  *you've* launched, not everyone sharing this namespace. The form's
