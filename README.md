...
`ctr.int.techboredom.com:8443/aether` on every push to `main` (except
...
3. **Apply the manifests** (one-time bootstrap only — after Argo CD's
   ...
   - `Service` (`aether`) — `type: LoadBalancer`, port `3000`, gets an
     external IP from the cluster's MetalLB pool (`192.168.10.0/24`).
...
2. Trust this Forgejo host's SSH key (Argo CD ships known-hosts entries for
   ...
   kubectl create secret generic aether-repo-creds -n argocd \
     ...
     --from-literal=url=ssh://git@quasar.techboredom.com:2022/Aether/Aether-Web.git \
...
to `main` → CI builds and pushes `ctr.int.techboredom.com:8443/aether:<sha>`
