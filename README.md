...
- **Repo**: `ssh://git@quasar.techboredom.com:2022/Aether/Aether-Web.git`
  (app code — this repo). Kubernetes manifests and the Argo CD `Application`
  live in a separate
  [**Aether-Deploy**](https://quasar.techboredom.com/Aether/Aether-Deploy)
...
| `--app-origin` / `APP_ORIGIN` | *(none)* | Public origin this app is served from, e.g. `https://aether.int.techboredom.com`. Must be set together with `--proxy-base-domain` |
| `--proxy-base-domain` / `PROXY_BASE_DOMAIN` | *(none)* | Base domain for per-deployment proxy origins, e.g. `proxy.aether.int.techboredom.com`. Needs wildcard DNS + TLS for `*.<domain>`. **Leaving it unset is only appropriate for local development** — see "Per-deployment proxy origins" below |
...
image to `ctr.int.techboredom.com:8443/aether/aether` (the `aether` project
...
- **Docker**, obviously, plus TLS trust for the Harbor registry — Docker's
  ...
  at `/etc/docker/certs.d/ctr.int.techboredom.com:8443/ca.crt` (directory
...
separate [**Aether-Deploy**](https://quasar.techboredom.com/Aether/Aether-Deploy)
...
- **Image pull secret** — the registry requires auth:
  ...
  kubectl create secret docker-registry regcred \
    --docker-server=ctr.int.techboredom.com:8443 \
...
2. Trust this Forgejo host's SSH key (Argo CD ships known-hosts entries for
   ...
   ```
   ssh-keyscan -p 2022 quasar.techboredom.com
   ```
   ...
   **Aether-Deploy** repo, and store the private half as a repository
   ...
   kubectl create secret generic aether-deploy-repo-creds -n argocd \
     ...
     --from-literal=url=ssh://git@quasar.techboredom.com:2022/Aether/Aether-Deploy.git \
...
`ctr.int.techboredom.com:8443/aether/aether:<sha>` → CI clones Aether-Deploy,
